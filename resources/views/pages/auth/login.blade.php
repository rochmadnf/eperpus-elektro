...
@section('content')
    <main class="flex min-h-screen w-full items-center justify-center bg-slate-50 bg-circuit-slate-300">
        <div class="space-y-6 rounded-xl bg-white p-6 shadow-md shadow-slate-500/50">
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                @csrf
                <x-input placeholder="rochmadnf" name="username" label="Nama Pengguna" autofocus tabindex="1" />
                <x-input placeholder="******" name="password" type="password" label="Katasandi" tabindex="2" />
